...
class BitLoader(ABC):
    ...
    @staticmethod
    def apply(ibr: IntArray, Msvpb: int, Ns: IntArray) -> tuple[list[list[BitArray]], list[BitArray]]:
        """
        ...
        Returns
        -------
        tx_bits_list : list[list[BitArray]], shape (K, Ns[k], ibr[k][s]*Msvpb)
            The list of allocated bits for each data stream of each UT.
        b : list[BitArray], shape (Ns_total, ibr[s]*Msvpb)
        ...
        for s in range(len(ibr)):
            if ibr[s] > 0:
                b_s = np.random.randint(0, 2, size=(ibr[s] * Msvpb,), dtype=np.uint8)
                b.append(b_s)
